...
class CompactWorkspaceTests(unittest.TestCase):
 ...
 def test_tools_synthesizes_system_and_advanced_access(self):
  self.assertIn('super().__init__("Tools & Setup"',self.pages);self.assertIn('QPushButton("Recheck Processing Engine")',self.pages);self.assertIn('QPushButton("Open Diagnostics")',self.pages)
