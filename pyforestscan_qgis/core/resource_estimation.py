...
def estimate_work_unit_resources(point_count,*,point_item_size=48,array_copies=3,hag_method="existing_normalized_height",raster_cells=0,available_memory=8*1024**3,core_width=750.0,product="chm"):
    points=max(0,int(point_count));point_bytes=int(points*estimated_point_memory_bytes(point_item_size=point_item_size,array_copies=array_copies,hag_method=hag_method));raster_arrays=5 if product=="rumple" else 3;raster_bytes=int(raster_cells*4*raster_arrays);total=point_bytes+raster_bytes+128*1024**2
