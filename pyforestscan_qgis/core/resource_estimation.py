...
def estimate_work_unit_resources(point_count,*,point_item_size=48,array_copies=3,hag_method="existing_normalized_height",raster_cells=0,available_memory=8*1024**3,core_width=750.0):
    points=max(0,int(point_count));triangulation=3.0 if hag_method=="classified_ground_delaunay" else 0.35
    native_multiplier=1.75;point_bytes=int(points*point_item_size*(array_copies+native_multiplier+triangulation));raster_bytes=int(raster_cells*4*3);total=point_bytes+raster_bytes+128*1024**2
