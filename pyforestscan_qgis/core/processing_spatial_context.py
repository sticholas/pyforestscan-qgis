...
class SourceLocalFallbackPolicyStore:
    ...
    def read(self) -> SourceLocalFallbackPolicy:
        try:
            ...
            return SourceLocalFallbackPolicy(choice, int(value.get("version", 1)))
    ...
    def write(self, policy: SourceLocalFallbackPolicy) -> None:
        ...
        temporary.write_text(json.dumps({"version": policy.version, "source_local_default_units": policy.default_units.value}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
__all__ = [
    "ProcessingSpatialContext", "SOURCE_LOCAL_FALLBACK_PRODUCTS", "SourceLocalFallbackChoice",
    "SourceLocalFallbackPolicy", "SourceLocalFallbackPolicyStore", "SpatialUnitBasis",
    "default_source_local_policy_store", "processing_spatial_context_from_dict", "resolve_processing_spatial_context",
]
