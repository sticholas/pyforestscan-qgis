...
| Environment Check | Overall `READY` is expected when PBM is Ready and QGIS Python scientific packages are absent; QGIS Python packages appear under optional fallback status and do not block routed products. |
...
- PBM install result:
- PBM verify result:
- Environment Check result (`READY` expected if PBM is Ready, with QGIS Python deps shown as optional fallback status):
