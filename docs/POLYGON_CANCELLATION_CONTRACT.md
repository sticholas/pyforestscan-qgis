...
Cancel and pause files belong to one coordinator attempt directory. User cancellation records `cancel_origin=USER`; launcher return never creates cancellation state. Stale control files from an earlier attempt cannot affect a new coordinator.
