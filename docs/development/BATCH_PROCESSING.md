...
Batch Processing v1 adds a folder-to-products workflow for users who need to run the same product plan across multiple lidar datasets without repeating the single-file Mission Control workflow by hand.
...
Raster products generated by batch jobs use the same QGIS result loading and styling path as single-file processing because job updates are emitted through the existing Mission Control job handler.

## Validation Notes

Batch v1 intentionally runs on the QGIS main thread. This keeps the orchestration simple and conservative for the first production slice, but large batches may make QGIS feel busy until a future asynchronous worker phase.
