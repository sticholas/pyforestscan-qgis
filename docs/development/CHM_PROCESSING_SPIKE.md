# CHM Processing Spike Manual Testing

Phase 10A enables one real scientific processing path: Canopy Height Model (CHM)
generation for a single small lidar dataset. All other planned products remain
not implemented.
...
2. `pyforestscan.calculate_chm(point_array, (grid_resolution, grid_resolution),
   interpolation="linear", interp_valid_region=False, interp_clean_edges=False)`
...
`job_summary.json` records the CHM GeoTIFF as a `chm_geotiff` result when the
job succeeds.
...
6. On Planning, leave only `Canopy Height Model (CHM)` selected for the first
   test, choose a conservative grid resolution such as `1.0`, and build the
   plan.
7. On Processing, choose `Start CHM Job`.
8. Confirm the pipeline reaches `completed` and `outputs/chm.tif` exists.
...
11. Open `logs/job_summary.json` and confirm:
    ...
    - a `chm_geotiff` result points to `outputs/chm.tif`
...
- No custom interpolation controls are exposed in Mission Control yet.
- No styling, symbology, pyramids, statistics, or publication layout is applied.
