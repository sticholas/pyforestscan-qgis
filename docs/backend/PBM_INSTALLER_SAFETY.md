...
Micromamba creates the conda-forge environment from the backend spec. The spec keeps `pip` available but does not use an inline `pip:` section. GDAL, libgdal, PDAL, python-pdal, rasterio, numpy, scipy, pandas, shapely, pyproj, fiona, geopandas, matplotlib, and tqdm are conda-forge owned. Registry entries sourced from PyPI, currently `pyforestscan>=0.4`, are installed afterward with `<backend env>/python -m pip install --no-deps ...` using the same sanitized environment. That prevents pip from replacing conda-forge geospatial/scientific DLLs or Python extension modules. The installer must never call QGIS Python pip. Package/import mapping is audited in [PBM Internal Beta Troubleshooting](PBM_INTERNAL_BETA_TROUBLESHOOTING.md).
