...
| Product-level crop/bounds on every metric | `read_lidar(... bounds/crop_poly/poly ...)` | Partial | HAG/Normalize exposes low-level read options; product tools do not yet have a consistent crop UX. | Add crop controls once vector/bounds selection is designed across guided and advanced workflows. |
