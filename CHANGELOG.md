...
- Phase 23M corrects Environment Check readiness messaging so QGIS Python scientific package failures are reported separately from PBM backend readiness. When PBM is READY, routed products report `READY WITH PBM BACKEND` instead of `NOT READY`, with explicit active-backend, product-coverage, and next-step guidance.
