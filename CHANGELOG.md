...
- Phase 23M corrects Environment Check readiness messaging so QGIS Python scientific package failures are reported separately from PBM backend readiness. When PBM is READY, routed products report `READY WITH PBM BACKEND` instead of `NOT READY`, with explicit active-backend, product-coverage, and next-step guidance. PBM installation now runs from a Qt worker with estimated staged progress, elapsed-time/current-action UI, hidden technical logs, disabled install/repair controls while running, and Windows no-console subprocess flags.
